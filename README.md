...
[![Build Status](https://travis-ci.org/endurio/neutrino.svg?branch=master)](https://travis-ci.org/endurio/neutrino)
[![Godoc](https://godoc.org/github.com/endurio/neutrino?status.svg)](https://godoc.org/github.com/endurio/neutrino)
[![Coverage Status](https://coveralls.io/repos/github/endurio/neutrino/badge.svg?branch=master)](https://coveralls.io/github/endurio/neutrino?branch=master)
...
#### Rescan
`Rescan` allows a wallet to scan a chain for specific TXIDs, outputs, and addresses. A start and end block may be specified along with other options. If no end block is specified, the rescan continues until stopped. If no start block is specified, the rescan begins with the latest known block. While a rescan runs, it notifies the client of each connected and disconnected block; the notifications follow the [chainjson](https://github.com/endurio/ndrd/blob/master/chainjson/chainsvrwsntfns.go) format with the option to use any of the relevant notifications. It's important to note that "recvtx" and "redeemingtx" notifications are only sent when a transaction is confirmed, not when it enters the mempool; the client does not currently support accepting 0-confirmation transactions.
